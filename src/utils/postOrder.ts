...
import { sendTradeNotification, sendErrorNotification } from '../services/telegramBot';
...
const postOrder = async (
    ...
    if (condition === 'merge') {
        ...
        if (totalBoughtTokens > 0) {
            ...
            // Send Telegram notification for successful buy
            sendTradeNotification(userAddress, 'BUY', {
                title: trade.title,
                outcome: trade.outcome,
                amount: orderCalc.finalAmount - remaining,
                price: trade.price,
                success: true,
            }).catch(() => {});
        } else if (abortDueToFunds || retry >= RETRY_LIMIT) {
            // Send Telegram notification for failed buy
            sendTradeNotification(userAddress, 'BUY', {
                title: trade.title,
                outcome: trade.outcome,
                ...
                price: trade.price,
                success: false,
                errorMessage: abortDueToFunds ? 'Insufficient balance' : 'Max retries reached',
            }).catch(() => {});
        }
        ...
        if (abortDueToFunds) {
            ...
            // Send Telegram notification for failed sell
            sendTradeNotification(userAddress, 'SELL', {
                title: trade.title,
                outcome: trade.outcome,
        ...
        if (retry >= RETRY_LIMIT) {
            ...
            // Send Telegram notification for successful sell
            if (totalSoldTokens > 0) {
                sendTradeNotification(userAddress, 'SELL', {
                    title: trade.title,
                    outcome: trade.outcome,
                    amount: totalSoldTokens * trade.price,
                    price: trade.price,
                    success: true,
                }).catch(() => {});
            }
